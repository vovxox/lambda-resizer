...
def is_resized_image_exists(bucket_name, key, size):
    try:
        s3.Object(
            bucket_name=bucket_name, key="{size}_{key}".format(size=size, key=key)
        ).load()
...
def resize_image(bucket_name, key, size):
    ...
    return "https://{bucket}.s3.amazonaws.com/{resized_key}".format(
        bucket=bucket_name, resized_key=resized_key
    )
...
def lambda_handler(event, context):
    ...
    return {"statusCode": 301, "body": image_s3_url}
